      ...
      .then(resp => {
        ...
        if (resp.status !== 200) {
          switch (resp.status) {
            case 401:
              // 记录登陆前的路由地址
              // global.sessionStorage.setItem('pathBeforeLogin', router.app.$route.path)
              // router.go({ name: 'login' })
              if (IS_DEBUG) {
                console.log('请按F9 进行登录')
              } else {
                // 暂时注释 todo
                /* if (store.state.account.baseUrl) {
                  global.location.href = store.state.account.baseUrl + '/login'
                  // 为了不让后续所有的then和catch都不执行,返回一个promise, 永远不resolve或reject
                } else {
                  get('/goods/domain/login')
                    .then(res => {
                      global.location.href = res.result + '/login'
                    })
                }*/
                return new Promise(() => {})
              }
